...
export default function RecentActivity({
  ...
  const getIconConfig = (type: string) => {
    switch (type) {
      case "contact":
        return { icon: UserPlus, color: "text-blue-500 dark:text-blue-400", bgColor: "bg-blue-500/10 dark:bg-blue-500/15" };
      case "deal":
        return { icon: Briefcase, color: "text-emerald-500 dark:text-emerald-400", bgColor: "bg-emerald-500/10 dark:bg-emerald-500/15" };
      case "task":
        return { icon: CheckCircle2, color: "text-purple-500 dark:text-purple-400", bgColor: "bg-purple-500/10 dark:bg-purple-500/15" };
      default:
        return { icon: Clock, color: "text-muted-foreground", bgColor: "bg-muted/50 dark:bg-muted/25" };
    }
  };
  ...
  return (
    <Card className="border shadow-sm rounded-2xl overflow-hidden">
      ...
      <CardContent>
        <div className="relative space-y-6 before:absolute before:left-5 before:top-2 before:bottom-2 before:w-0.5 before:bg-muted/50">
          {activities.length === 0 ? (
